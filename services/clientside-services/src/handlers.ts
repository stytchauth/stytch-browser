...
export const getExamplePhoneNumber: RPCManifest['getExamplePhoneNumber'] = async ({ regionCode }) => {
  const { getExample } = await import('awesome-phonenumber');
  const example = getExample(regionCode, 'mobile');
  return {
    phoneNumber: example.getNumber('national'),
  };
};
