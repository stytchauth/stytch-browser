import { B2BProducts, StytchEventType, AuthFlowType } from '@stytch/core/public';
...
describe('Discovery Flow', () => {
  ...
  it('handles SSO-only organization click', async () => {
    ...
    await renderFlow({
      config: {
        ...config,
        products: [B2BProducts.emailMagicLinks, B2BProducts.sso, B2BProducts.passwords],
      },
      client,
    });
  ...
  it('handles unauthenticated discovery (SSO + passwords)', async () => {
    ...
    // Click on the organization
    await userEvent.click(screen.getByText('Mock Org'));
