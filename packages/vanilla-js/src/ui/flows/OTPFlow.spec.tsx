...
import { OTPMethods, Products, StytchAPIError } from '@stytch/core/public';

import Container from '../b2c/Container';
import { MockConfig, MockGlobalContextProvider, MockClient, MockState, render, screen, waitFor } from '../../testUtils';
...
describe('OTP Flow', () => {
  ...
  it('Successfully handles a sms otp login', async () => {
    ...
    await userEvent.type(screen.getByPlaceholderText('(123) 456-7890'), MOCK_PHONE);

    await waitFor(async () => {
      screen.getByText('Continue with text message');
    });

    await userEvent.click(screen.getByText('Continue with text message'));
  ...
  it('Successfully handles a whatsapp otp login', async () => {
    ...
    await userEvent.click(screen.getByText('Continue with WhatsApp'));
