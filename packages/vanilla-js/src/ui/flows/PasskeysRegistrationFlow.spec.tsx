...
import { Callbacks, Products, WebAuthnRegistration } from '@stytch/core/public';
...
describe('Passkey Registration Flow', () => {
  ...
  beforeEach(async () => {
    client = new StytchUIClient('public-token-fake-public-token');
  ...
  const clickPasskeyAction = async (row: HTMLElement, label: string) => {
    await userEvent.click(within(row).getByLabelText(label));
  };

  const clickCreateAPasskey = async () => {
    await userEvent.click(screen.getByText('Create a Passkey'));
  };
  ...
  //  Success Page Tests
  const updateFailureHandler = Object.values(passkeyUpdateErrorHandlers({ networkDelay: 3 }));
  ...
  describe('editing passkey', () => {
    ...
    it('successfully edits a passkey name', async () => {
      ...
      expect(getInput(registrationToEdit).disabled).toBe(true);
      ...
      expect(getInput(registrationToEdit).value).toEqual(NEW_NAME);
      expect(getInput(registrationToEdit).disabled).toBe(true);
    });
