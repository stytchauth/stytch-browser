...
export const ZxcvbnWithSuggestions = {
  play: async ({ canvas }) => {
    ...
    await expect(await canvas.findByText('No need for symbols, digits, or uppercase letters.')).toBeInTheDocument();
  },
} satisfies Story;
...
export const LudsValid = {
  play: async ({ canvas }) => {
    ...
    // The color checks here are very fragile and not ideal, but it's the best
    // we have as of this writing. In the future we should make this more
    // accessible and leverage that in these tests.
    await expect(await canvas.findByText('Must be at least 8 characters long')).toHaveAttribute('color', 'success');
    await expect(
      await canvas.findByText('Must contain 3 of the following: uppercase letter, lowercase letter, number, symbol'),
...
export const LudsMissingCharacters = {
  play: async ({ canvas }) => {
    ...
    await expect(await canvas.findByText('Must be at least 8 characters long')).toHaveAttribute('color', 'error');
    await expect(
      await canvas.findByText('Must contain 3 of the following: uppercase letter, lowercase letter, number, symbol'),
    ).toHaveAttribute('color', 'success');
