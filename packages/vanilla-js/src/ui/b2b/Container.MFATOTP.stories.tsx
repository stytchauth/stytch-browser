...
export const EnrollmentManual = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await waitFor(() => userEvent.click(canvas.getByText('Having trouble scanning?')));

    await waitFor(() => expect(canvas.getByText('Having trouble scanning the QR code?')).toBeInTheDocument());
    await expect(
      canvas.getByText(
        (_, element) => element?.nodeName === 'CODE' && element?.textContent === 'wg2sak2fpopzrcwlomelwtbcpaegbfuo',
      ),
    ).toBeInTheDocument();
  },
} satisfies Story;

export const EnrollmentManualBack = {
  ...EnrollmentManual,
  play: async (params) => {
...
export const EnrollmentManualScanAgain = {
  ...
  play: async (params) => {
    ...
    await userEvent.click(canvas.getByText('Try to scan the QR code again'));
    await waitFor(() =>
      expect(canvas.getByText('Scan the QR code to link your authenticator app')).toBeInTheDocument(),
    );
  },
...
export const EntrySwitchToSMSOTP = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await waitFor(() => expect(canvas.getByText('Enter verification code')).toBeInTheDocument());
    await userEvent.click(canvas.getByText('Text me a code instead'));
...
export const EntryBackupCode = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await waitFor(() => expect(canvas.getByText('Enter verification code')).toBeInTheDocument());
    await userEvent.click(canvas.getByText('Use a backup code'));
...
export const EntryBackupCodeInvalid = {
  ...
  play: async (params) => {
    ...
    await waitFor(() => expect(canvas.getByText('Continue')).toBeEnabled());
    await userEvent.click(canvas.getByText('Continue'));
