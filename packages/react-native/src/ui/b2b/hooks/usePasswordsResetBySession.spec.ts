...
describe('usePasswordResetBySession', () => {
  ...
  it('dispatches expected error when password is missing', async () => {
    ...
    const mockStytchClient = {
      passwords: {
        resetBySession: jest.fn().mockRejectedValue(MOCK_STYTCH_API_ERROR),
      },
    } as unknown;
    jest.spyOn(ContextProvider, 'useStytch').mockReturnValue(mockStytchClient as StytchB2BClient);
    const { passwordResetBySession } = usePasswordResetBySession();
    await passwordResetBySession();
    expect(dispatchMock).toHaveBeenCalledWith({
      type: 'passwords/resetBySession/error',
  ...
  it('dispatches expected success when call succeeds', async () => {
    ...
    const mockStytchClient = {
      passwords: {
        resetBySession: jest.fn().mockResolvedValue(MOCK_MEMBER_NEEDS_MFA),
      },
    } as unknown;
    jest.spyOn(ContextProvider, 'useStytch').mockReturnValue(mockStytchClient as StytchB2BClient);
    const { passwordResetBySession } = usePasswordResetBySession();
