  ...
  implements INativeOAuthClient<TProjectConfiguration>
{
  ...
  authenticate = this._subscriptionService.withUpdateSession(
    async (
      ...
      await this._pkceManager.clearPKPair();
  ...
  private async createOAuthURL(
    ...
    if (custom_scopes) {
      validate('createOAuthURL').isStringArray('custom_scopes', custom_scopes);
      queryParams['custom_scopes'] = custom_scopes.join(' ');
    }

    if (provider_params) {
      validate('createOAuthURL').isOptionalObject('provider_params', provider_params);
