...
describe('HeadlessWebAuthnClient', () => {
  ...
  describe('authenticate', () => {
    ...
    it('should skip input check when disable_input_check is true with conditional mediation', async () => {
      const options = {
        session_duration_minutes: 60,
        conditional_mediation: true,
      ...
      jest.spyOn(client, 'browserSupportsAutofill').mockResolvedValueOnce(true);
      const checkEligibleInputsSpy = jest.spyOn(client as any, 'checkEligibleInputs');

      await client.authenticate(options);

      expect(checkEligibleInputsSpy).not.toHaveBeenCalled();

      // Verify conditional mediation was still used
      expect(webauthnJson.get).toHaveBeenCalledWith(expect.objectContaining({ mediation: 'conditional' }));
    });

    it('Should return null when input check fails and disable_input_check is false', async () => {
      const options = {
        session_duration_minutes: 60,
        conditional_mediation: true,
      ...
      // Mock out logger for now to reduce noise
      const loggerErrorSpy = jest.spyOn(logger, 'error').mockImplementation(() => {
        // noop
      });

      jest.spyOn(client, 'browserSupportsAutofill').mockResolvedValueOnce(true);
      jest.spyOn(client as any, 'checkEligibleInputs').mockReturnValueOnce(false);
