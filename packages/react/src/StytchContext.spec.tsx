...
import { Session, StytchUIClient, User } from '@stytch/vanilla-js';
...
} satisfies PartialDeep<jest.Mocked<StytchHeadlessClient>>;
...
describe('StytchContext', () => {
  ...
  const renderWithProvider = (
    ui: ReactElement,
    { stytch = mockStytchClient as unknown as StytchHeadlessClient, ...rest }: Partial<StytchProviderProps> = {},
  ) =>
    render(
  ...
  it('normal usage', async () => {
    const { unmount } = render(<StytchProvider stytch={mockStytchClient as unknown as StytchHeadlessClient} />);
  ...
  it('supports assumeHydrated = false', async () => {
    mockClientInfo({ user: testUser, session: null, fromCache: true });
    renderWithProvider(<UseStytchUserTest />, { assumeHydrated: false });
  ...
  it('useStytchSession passes the Stytch Session into the component', async () => {
    ...
    expect(captureValue).toHaveBeenCalledTimes(1);
    expect(captureValue).toHaveBeenCalledWith({ fromCache: true, session: testSession, isInitialized: true });
    captureValue.mockClear();

    mockClientInfo({ user: testUser, session: testSession, fromCache: false });
    renderWithProvider(<UseStytchSessionTest />);
    expect(captureValue).toHaveBeenCalledTimes(1);
    expect(captureValue).toHaveBeenCalledWith({ fromCache: false, session: testSession, isInitialized: true });
  });
  ...
  it('Login and Logout events cause state changes inside the components', async () => {
    ...
    expect(captureValue).toHaveBeenLastCalledWith({
      userReturn: { fromCache: true, user: null, isInitialized: true },
      sessionReturn: { fromCache: true, session: null, isInitialized: true },
    });
    ...
    expect(captureValue).toHaveBeenLastCalledWith({
      userReturn: { fromCache: false, user: testUser, isInitialized: true },
      sessionReturn: { fromCache: false, session: testSession, isInitialized: true },
    });
    ...
    expect(captureValue).toHaveBeenLastCalledWith({
      userReturn: { fromCache: false, user: null, isInitialized: true },
      sessionReturn: { fromCache: false, session: null, isInitialized: true },
    });
  });

  it('no provider errors', async () => {
    expectToThrow(() => render(<UseStytchTest />), noProviderError('useStytch'));
    expectToThrow(() => render(<WithStytchTest />), noProviderError('withStytch'));
    expectToThrow(() => render(<UseStytchUserTest />), noProviderError('useStytchUser'));
    expectToThrow(() => render(<WithStytchUserTest />), noProviderError('withStytchUser'));
    expectToThrow(() => render(<UseStytchSessionTest />), noProviderError('useStytchSession'));
    expectToThrow(() => render(<WithStytchSessionTest />), noProviderError('withStytchSession'));
  });
  ...
  it('nested stytch context errors', async () => {
    expectToThrow(
      () =>
        render(
          <StytchProvider stytch={mockStytchClient as unknown as StytchUIClient}>
            <StytchProvider stytch={mockStytchClient as unknown as StytchUIClient} />
          </StytchProvider>,
        ),
      new Error('You cannot render a <StytchProvider> inside another <StytchProvider>.'),
    );
  });
  ...
  it('useStytchIsAuthorized returns whether the user is authorized', async () => {
    ...
    expect(captureValue).toHaveBeenCalledWith({ fromCache: false, isInitialized: false, isAuthorized: false });
