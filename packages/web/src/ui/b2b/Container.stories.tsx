...
import { emailMagicLinks, oauth, passwords, sso } from './B2BProducts';
import Container from './Container';
import { AppScreens } from './types/AppScreens';

const b2bConfig = {
  products: [oauth, sso, emailMagicLinks, passwords],
  oauthOptions: {
    providers: [
...
} as Partial<StytchB2BExtendedLoginConfig>;
...
export const DiscoveryLogin = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByText('Continue with Slack')).toBeInTheDocument();
    await expect(canvas.getByText('Continue with GitHub')).toBeInTheDocument();
    await expect(canvas.getByText('Use single sign-on')).toBeInTheDocument();
  ...
  parameters: {
    stytch: {
      b2b: {
        ...
        config: {
          products: [sso, emailMagicLinks, oauth],
        },
      },
    },
  },
