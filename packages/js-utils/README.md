...
- Small enough that duplication is immaterial, or only used in such a way that duplication is not a concern.
  - For example, if code is shared between the `@stytch/react` and `@stytch/nextjs` packages, duplication is not a practical concern because consumers would only be expected to use one of those packages at a time.
